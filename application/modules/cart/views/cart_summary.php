<div class="cartSummary">
    ...
    {
        echo '<div class="alert">'.lang('empty_view_cart').'</div>';
    }
        ...
        <div class="cartItem">
                ...
                <div class="col" data-cols="4/5">
                    ...
                    if(isset($options[$product->id]))
                    {
                        foreach($options[$product->id] as $option):?>
                            <div class="cartItemOption"><?php echo $option->option_name;?> : <?php echo($option->price > 0)?'['.format_currency($option->price).']':'';?> <?php echo $option->value;?></div>
                        <?php endforeach;
                    }
                    ?>
                </div>
            ...
            <div class="col-nest">
                <div class="col" data-cols="1/2">
                    <strong><small><?php echo $product->quantity.' @ '.format_currency($product->total_price)?></small></strong>
                </div>
    ...
    <div class="cartPromotions">
        ...
        <div class="btn-group input-sm">
            ...
            <div><a class="btn green" type="button" onclick="submitCoupon()"><i class="icon-plus"></i></a></div>
        </div>
        <?php if(config_item('gift_cards_enabled')):?>
        ...
        <div class="btn-group input-sm">
            ...
            <div><a class="btn green" type="button" onclick="submitGiftCard()"><i class="icon-plus"></i></a></div>
        </div>
        <?php endif;?>
...
function updateItem(id, newQuantity, oldQuantity)
{
    if(newQuantity != oldQuantity)
    {
        ...
        $.post('<?php echo site_url('cart/update-cart');?>', {'product_id':id, 'quantity':newQuantity}, function(data){
            ...
            {
                $('.cartSummary').prepend($('<div class="alert red"></div>').text(data.error));
            }
