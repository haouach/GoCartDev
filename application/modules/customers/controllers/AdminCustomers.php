...
class AdminCustomers extends Admin {
    ...
    {
        $data['customers'] = (array)\CI::Customers()->get_customers();
        
        \CI::load()->helper('download_helper');
        force_download_content('customers.xml', $this->partial('customers_xml', $data, true));
    }
    ...
    public function order_list($status = false)
    {
        //we're going to use flash data and redirect() after form submissions to stop people from refreshing and duplicating submissions
        \CI::load()->model('Orders');
        
        $data['page_title'] = 'Order List';
        $data['orders'] = $this->Orders->getOrders($status);
    ...
    {
        $subscribers = \CI::Customers()->get_subscribers();
        ...
        $this->partial('customer_subscriber_list', $data);
    }   
    ...
    public function group_form($id=0)
    {
        \CI::load()->helper('form');
        \CI::load()->library('form_validation');
        ...
        \CI::form_validation()->set_rules('discount', 'lang:discount', 'trim|required|numeric');
        \CI::form_validation()->set_rules('discount_type', 'lang:discount_type', 'trim|required');
    ...
    public function get_group()
    {
        ...
        echo json_encode(\CI::Customers()->get_group($id));
    }
    ...
    public function address_list($customer_id)
    {
    ...
    public function address_form($customer_id, $id = false)
    {
        $data['id'] = $id;
        $data['company'] = '';
        $data['firstname'] = '';
    ...
    public function delete_address($customer_id = false, $id = false)
    {
        if ($id)
        {   
            $address = \CI::Customers()->get_address($id);
