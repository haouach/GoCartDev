...
class MyAccount extends Front {
    ...
    public function check_email($str)
    {
        ...
        if ($email)
        {
            \CI::form_validation()->set_message('check_email', lang('error_email'));
            return FALSE;
        }
        else
        {
    ...
    public function downloads($code=false)
    {
        
        if($code!==false)
        {
            $data['downloads'] = \CI::DigitalProducts()->get_downloads_by_code($code);
        } else {
            \CI::Login()->isLoggedIn();
            
            $customer = \CI::Login()->customer();
            
            $data['downloads'] = \CI::DigitalProducts()->get_user_downloads($customer['id']);
        }
        
        $data['page_title'] = lang('my_downloads');
        
        $this->view('downloads', $data);
    }
